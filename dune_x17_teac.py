...
p_pi_signal = np.sqrt((mK**2 + mPi**2 - mX**2)**2 / (4*mK**2) - mPi**2) # ~227 MeV

# Gera eixo de momento
p_pi = np.linspace(200, 240, 1000)
bin_width = p_pi[1] - p_pi[0] # tamanho de cada bin

# Sinal: gaussiana centrada em 227 MeV
sigma_res = 1.5 # MeV, resolução do DUNE
signal = BR_Kp_piX * N_K_plus * np.exp(-0.5 * ((p_pi - p_pi_signal) / sigma_res)**2)
signal = signal / (np.sqrt(2*np.pi) * sigma_res) # normaliza para eventos/MeV

# Background: K+ -> pi+ pi0, cauda
bkg = 1e11 * np.exp(-(p_pi - 205) / 5.0) # aumentado pra ficar visível
...
total_eventos = np.sum(signal) * bin_width # CORRIGIDO
plt.text(221, np.max(total)*0.8, f'~{total_eventos:.1e} eventos/ano\nSignificância > 50$\sigma$',
         bbox=dict(facecolor='white', alpha=0.8))

plt.tight_layout()
plt.savefig('dune_X17_sens.png', dpi=300)
print(f"Gráfico salvo. Pico em {p_pi_signal:.1f} MeV com {total_eventos:.1e} eventos/ano")
